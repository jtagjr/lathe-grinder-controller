...
@ingroup setup
*/
void ModbusMaster::begin(uint8_t slave, UnbufferedSerial &serial)
{
...
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  printf("Modbus write function %u\n", u8MBFunction);
  switch(u8MBFunction)
  {
    case ku8MBReadCoils:
  ...
  while (u8BytesLeft && !u8MBStatus)
  {
...
#endif
      uint8_t byte;
      if (_serial->read(&byte, 1))
      {
          u8ModbusADU[u8ModbusADUSize++] = byte;
                /* Fixed Jtag  u8ModbusADU[u8ModbusADUSize++] = _serial->read(&byte, 1); */
        u8BytesLeft--;
      }
    ...
    if (u8ModbusADUSize == 5)
    {
      ...
      if ((u8ModbusADU[1] & 0x7F) != u8MBFunction)
      {
        ...
        printf("Modbus function recv: %#02X:%#02X:%#02X:%#02X:%#02X\n", u8ModbusADU[0], u8ModbusADU[1], u8ModbusADU[2], u8ModbusADU[3], u8ModbusADU[4]);
        break;
      }
    ...
    if (std::chrono::duration_cast<std::chrono::milliseconds>(timer.elapsed_time()).count() > ku16MBResponseTimeout)
    {
...
const char* ModbusMaster::ModbusErrorString(uint8_t status)
{
    static const char* ku8MBSuccess = "success";
    static const char* ku8MBIllegalFunction = "illegal function";
    static const char* ku8MBIllegalDataAddress = "illegal data address";
    static const char* ku8MBIllegalDataValue = "illegal data value";
    static const char* ku8MBSlaveDeviceFailure = "slave device failure";
    static const char* ku8MBInvalidSlaveID = "invalid slave id";
    static const char* ku8MBInvalidFunction = "invalid function";
    static const char* ku8MBResponseTimedOut = "slave response timeout";
    static const char* ku8MBInvalidCRC = "invalid slave crc";
    static const char* unknownErrorCode = "unknown error code";
    ...
    switch (status)
    {
        ...
        case ModbusMaster::ku8MBSlaveDeviceFailure:
            return ku8MBSlaveDeviceFailure;

        case ModbusMaster::ku8MBInvalidSlaveID:
            return ku8MBInvalidSlaveID;
